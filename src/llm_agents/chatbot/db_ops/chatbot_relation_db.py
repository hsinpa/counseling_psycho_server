...
def get_chatroom_message(session_id: str, limit: int):
    sql_syntax = f"""SELECT * FROM {DB_TABLE_MESSAGES} WHERE session_id = %s LIMIT %s"""

    sql_result = sync_db_ops(sql_syntax=sql_syntax, fetch_type=FetchType.Many, parameters=[session_id, limit])
    ta = TypeAdapter(List[ChatMessage])

    return ta.validate_python(sql_result)

def insert_chatroom_message(session_id: str, user_id: str, bubble_id: str,
    ...
    sql_result = sync_db_ops(sql_syntax=sql_syntax,
                             parameters=[user_id, session_id, bubble_id, ChatbotUserEnum.human, user_message,
                                         user_id, session_id, bubble_id, ChatbotUserEnum.bot, agent_message])